 ...
 */
function extractArgument(parameterName, message, parsedArguments, required = false) {
    ...
    if (matches == null) {
        if (required) { // If the argument is required
            throw new errors_1.MissingArgumentError(parameterName);
        }
    ...
    if (matches.length > 1) {
        throw new errors_1.TooManyArgumentsError(parameterName, matches);
    }
    let argumentValue;
    if (parameterName == 'course')
        ...
        throw new errors_1.ValueError('parameterName', parameterName, ['course', 'professor', 'section', 'semester']);
...
function parseCourse(input) {
    ...
    if (department == null || department.length > 1 || courseNumber == null || courseNumber.length > 1) {
        ...
        throw new errors_1.ParserError(input, additionalInfo);
    }
...
function parseSemester(input) {
    ...
    if (tempYear.match(/^\d\d(\d\d)?$/) == null) {
        throw new errors_1.ParserError(input, 'semester', `Could not extract a valid 2- or 4-digit year from remaining string '${tempYear}'.`);
    }
    // Join the tokens
    switch (tempSeason) {
        case 's':
